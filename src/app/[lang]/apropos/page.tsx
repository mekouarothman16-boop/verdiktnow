  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen">
      ...
      <main className="flex-1 max-w-[760px] mx-auto w-full px-5 sm:px-6 py-14">
        <h1 className="font-display text-[30px] sm:text-[36px] font-extrabold tracking-[-0.015em] text-ink mb-10">
          {t.title}
        </h1>

        <section className="mb-10">
          <h2 className="font-display text-[19px] font-bold text-ink mb-2.5">{t.whoTitle}</h2>
          <p className="text-[14.5px] text-ink-soft leading-relaxed">{t.whoText}</p>
        </section>

        <section className="mb-10">
          <h2 className="font-display text-[19px] font-bold text-ink mb-2.5">{t.missionTitle}</h2>
          <p className="text-[14.5px] text-ink-soft leading-relaxed">{t.missionText}</p>
        </section>

        <section className="mb-10">
          <h2 className="font-display text-[19px] font-bold text-ink mb-4">{t.valuesTitle}</h2>
          <div className="grid sm:grid-cols-2 gap-4">
            {t.values.map((v, i) => {
              const Icon = ICONS[i];
              return (
                <Card key={v.title} className="p-5">
                  <div className="w-9 h-9 rounded-[9px] bg-accent-soft flex items-center justify-center mb-3">
        ...
        <section>
          <h2 className="font-display text-[19px] font-bold text-ink mb-2.5">{t.historyTitle}</h2>
